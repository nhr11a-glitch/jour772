...
**Answer:**
#The column "expenditure_date" is now being registered as a date <date> instead of a character <chr>. The dates in the column maintained their full mm dd yyyy, but were rearranged to look like yyyy-mm-dd. 
...
**Answer**
# The dates in the month column maintained their full mm dd yyyy, but were rearranged to look like yyyy-mm-dd. All dates with the same month and year were grouped together under the umbrella of the first day of the month. The dates are NOT in descending order, because we commanded it to order the table in descending order of total amount. 
...
**Answer**
#It looks like we told it to mutate two new columns at once but add the changes to the existing table. We made it created a new column "total_votes" and gave it the specific columns to add <+> up. We then immediately followed with another mutate in the same commmand, signified by <,>. We commanded it to create another new column "pct_moore" and instructed it to create a new number by dividing </> columns "moore" by the "total_votes". 
...
**Answer**
#We commanded it to arrange the table in descending order of the new column "pct_moore": in order of counties with most percentage votes for Moore to the least. Before, it seemed to be in alphabetical order based on the county column. 
...
**Answer**
#Personally, I think ascending order is more interesting. My understanding is that Moore won by a pretty big margin, so it's more interesting to see the counties he didn't fare as well in. It introduces more questions, which I think is the point of visualizing data like this. 
...
**Answer**
#I asked Claude, "Can you give me tidyverse functions similar to str_to_upper?" 
#Claude gave me the following from the stringr package: 
#str_to_lower() - converts to lowercase
#str_to_title() - converts to title case (first letter of each word capitalized)
#str_to_sentence() - converts to sentence case (first letter capitalized, rest lowercase)
#str_trim() - removes whitespace from start and end
#str_squish() - removes extra whitespace and trims
#str_pad() - adds padding to strings
#str_trunc() - truncates strings to specified length
#str_wrap() - wraps strings to specified width
#str_replace() and str_replace_all() - replace patterns in strings
#str_remove() and str_remove_all() - remove patterns from strings
#I then asked Claude if stringr was part of tidyverse, and it confirmed it is part of core tidyverse. 
...
**Answer**
#We are mutating the table to produce a new column "state". We use the str command to detect when a state name pops up in the address column, giving it the exact names to look for, and then converting that value to a new abbreviated value for the "state" column. If none of the states identified show up, we are commanding it to list "NA." 
```{r}
...
**Answer**
#The answer makes sense except for the NA. It's unclear whether NA represents the other 41 states or addresses that didn't have states mentioned.
...
#1.  What percentage of Maryland expenses went to Maryland-based payees?

#2.  What do the Maryland expenses look like per year?

#3.  What would a list look that includes only UMD courses for the winter semester (202501) that require prerequisites or have restrictions? 
