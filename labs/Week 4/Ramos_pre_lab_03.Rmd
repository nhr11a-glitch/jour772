...
**Answer**
#The warning says "one or more parsing issues, call 'problems()' on your data frame for details." When I follow its instructions, it shows me there are problems in 369 of the rows, explaining that instead of an expected numeric value it got a character string.  
...
**Answer**
#I am not getting any parsing errors once we ran the codeblock using every row to set the data types. It's showing that in five of the columns we are getting character type information, and in the other three we get numeric values. 
...
**Answer**
#The vendor name field is comprised of characters, the amount field has numeric values, and dates are being registered as characters. 
...
**Answer**
# There are 553 zip codes are less than five characters in length. The four character codes could be stand alone postal codes (such as international ones), or the last four numbers in a Zip-4. 
...
**Answer**
#We are running a code asking to take md_grants_loans and group it by both Fiscal Year and Category columns, summarizing the payment count of both and arranging it by Fiscal Year. For example, the first line tells us that in 2009, there were 819 grant payments. The NA's in the Category column stand out to me, especially because payment(s) were still made in these years. 2013's Category of "L" is also weird. 
...
**Answer**
#What stands out to me are the first two grants listed, both less than $100. I don't know that I've ever seen a grant be that small of an amount. I also looked up easement because I didn't know what that meant, and apparently it means according to Oxford "a right to cross or otherwise use someone else's land for a specified purpose." I saw at least three payments that had "easement" included in the Description column. 
...
**Answer**
#The column names are now all lowercase. Column names with two words are separated with an underscore instead of a space. 
...
**Answer**
#We are creating a new dataframe called "cleaned_md_grants_loans" from the existing "md_grants_loans." We are following with a command to standardize all the names (clean_names) so they are all the same--lowercase and without spaces. Our next command is the rename the column "grantor" to the name "source." We follow that with a command to change (mutate) the characters to make the source, grantee and description columns upper-cased, using str_to_upper. We lastly want to see the new dataframe. 
...
**Answer**
#There are 58 possible duplicates in the data set. 
...
**Answer**
#There are now 19,453 rows on the new dataframe. 
...
**Answer**
#We've implemented all the previously discussed commands, adding on distinct() to get rid of duplicates. Using mutate and str_sub, we created a new column "zip5" that shows us a five character zip code for each line, pulling from the existing zip code data but limiting it to five characters. It still includes five character zips with letters and spaces, though, counting each as a separate character. 
...
**Answer**
#Zip code 21202, which includes the Inner Harbor area of downtown Baltimore recieved the most money from the Maryland Tourism Board. A quick Google AI search shows that this area is a major tourist hot spot with "cultural attractions, diverse dining, and waterfront views," as well as notable landmarks like "Inner Harbor, Edgar Allan Poe House and Museum, National Aquarium, Maryland Science Center." It does make sense that the Tourism Board would give this zip code the most amount of money, because of the potential ROI it could have and how many people frequenting the area. 
